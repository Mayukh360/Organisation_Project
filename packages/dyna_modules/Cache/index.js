 ...
 * GLOBALS
 */
const Client = new Redis({ 'connectTimeout': 10000 })
          ...
          if (this.configuration.changeTo === Array) {
            ...
            if (this.configuration && this.configuration.expireIn) {
              ...
              Interval(async (__number, __stop) => {
                // Const assignment.
                const _getDetails = await Client.get(id)
  ...
  hm({ id, cb }) {
    ...
    try {
      ...
      if (!_.isEmpty(id)) {
        ...
        // Return promise.
        return new Promise((__resolve, __reject) => Client.scanStream({ 'match': id, 'count': _this_.configuration.hm.count }).on('data', __data => {
          /*
