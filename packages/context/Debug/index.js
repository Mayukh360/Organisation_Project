 ...
 * IMPORTS
 */
import __debug from 'debug/src/node' // Npm: debug this module error with standalone color.
 ...
 * SIBLINGS
 */
import Middleware from './Middleware' // Custom: middleware Maps for handling custom methods.
import { errorHandlers } from './errors' // Custom: error handling.
    ...
    if (this instanceof Debug) {
      ...
      this.Logger = function Logger({ error, message, state }, $optionss) {
        ...
        try {
          ...
          if (global.WORKING_ENV.DEVELOPMENT.value === global.CONFIG_RC.env) {
            // Conditional Debugging.
            if (message) {
              // Standalone debugger.
              const debug = __debug(`${this.field}`)

              // Debug statement.
              debug(`--> ${state ?? 'Executing'} --> ${message}`)
            }
       ...
       * in one place.
       */
      this.QUEUE = [...errorHandlers, CONFIG_RC.env.toLowerCase().includes('dev') && this.Logger]
...
Debug.prototype = {
  ...
  'sentry': {
    ...
    'configuration': {
      ...
      'debug': global.WORKING_ENV.DEVELOPMENT.value === global.CONFIG_RC.env,
